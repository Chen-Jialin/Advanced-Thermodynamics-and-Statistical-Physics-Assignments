...
% paramagnetic phase
figure(1)
plot(x, x.^3/3, 'linewidth', 2)
hold on
plot(x, -x/3, '--', 'linewidth', 2)
legend('M^3/3', '(T_c/T-1)M')
...
% 把 matlab 做的图坐标轴移到图形的中间部分去(与数学的做图习惯一致)
...
set(temp_1,'XTick',[],'Color','None','YTick',[]) ;
set(temp_1,'YColor','w') ;
...
set(temp_2,'YTick', [],'Color','None','XTick',[]) ;
set(temp_2,'XColor','w') ;
...
text(Base_pos(1)+Base_pos(3), arrow_pos_in_x_dircetion - 4 * Base_pos(4), 'M', 'fontsize', 16)
text(-0.5 * Base_pos(1), - 4 * Base_pos(4), '0', 'fontsize', 16)
end